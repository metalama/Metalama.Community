...
internal static class Program
{
    ...
    private static int RunParent()
    {
        ...
        var childPath = CreateIsolatedCopy();
    ...
    /// the output directory, because the latter gives paths longer than <c>MAX_PATH</c>.
    /// </summary>
    private static string CreateIsolatedCopy()
    {
        var thisPath = new Uri( Assembly.GetExecutingAssembly().CodeBase! ).LocalPath;
        var directory = Path.Combine( Path.GetTempPath(), "Metalama.Community.Costura.ResolveRecursionTest" );
        ...
        var childPath = Path.Combine( directory, Path.GetFileName( thisPath ) );
